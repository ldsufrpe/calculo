...
\hypersetup{pdftitle={Cálculo Integral: N1}}
...
%
\typeout{************************************************}
\typeout{Seção 1 Integral Indefinida}
\typeout{************************************************}
%
...
\end{note}
\footnotetext[2]{saiba mais em \href{https://pt.wikipedia.org/wiki/Diferencial_(infinitesimal)}{wikipedia}\label{g:fn:idp10}}%
...
%
\typeout{************************************************}
\typeout{Subseção 1.4 Tecnologia}
\typeout{************************************************}
%
...
\end{paragraphs}%
\end{technology}
\footnotetext[3]{Linguagem de programação.\label{g:fn:idp40}}%
\footnotetext[4]{Uso gratuito e de código aberto.\label{g:fn:idp41}}%
\end{subsectionptx}
%
%
\typeout{************************************************}
...
\par
a) \(\displaystyle\int 4 e^t  \, dt\) = \fillin{13.6363636363636} \(+ C\).%
\par
b) \(\displaystyle\int 3^r  \, dr\) = \fillin{13.6363636363636} \(+ C\).%
\par
c) \(\displaystyle\int \dfrac{10^x}{18}  \,dx\) = \fillin{13.6363636363636} \(+ C\).%
\par\smallskip%
\noindent\textbf{\blocktitlefont Resposta 1}.\hypertarget{g:answer:idp44}{}\quad{}\(4e^{t}\)%
\par\smallskip%
\noindent\textbf{\blocktitlefont Resposta 2}.\hypertarget{g:answer:idp45}{}\quad{}\(\frac{3^{r}}{1.09861}\)%
\par\smallskip%
\noindent\textbf{\blocktitlefont Resposta 3}.\hypertarget{g:answer:idp46}{}\quad{}\(\frac{10^{x}}{18\cdot 2.30259}\)%
\par\smallskip%
...
\par\smallskip%
\noindent\textbf{\blocktitlefont Solução}.\hypertarget{g:solution:idp53}{}\quad{}Basta verificar que%
\begin{equation*}
...
%
\end{divisionexercise}%
\begin{divisionexercise}{4}{}{}{g:exercise:idp54}%
Considere a função \(\displaystyle f(x) = x^5 + 2 \sqrt{x}\).%
\par
Seja \(F(x)\) a primitiva de \(f(x)\) com \(F(1) = -6\).%
\par
Então \(F(x) =\) \fillin{13.6363636363636}%
\par\smallskip%
\noindent\textbf{\blocktitlefont Resposta}.\hypertarget{g:answer:idp55}{}\quad{}\(\frac{x^{6}}{6}+\frac{4x^{1.5}}{3}-7.5\)%
\par\smallskip%
\noindent\textbf{\blocktitlefont Solução}.\hypertarget{g:solution:idp56}{}\quad{}\(\displaystyle f(x) = x^5 + 2 \sqrt{x} = x^5 + 2 x^{1/2}\): \(\displaystyle F(x) = \frac{x^6}{6} + \frac{2 x^{3/2}}{3/2} + c  = 
\frac{x^6}{6} + \frac{4 x^{3/2}}{3} + c\) Já que \(F(1) = -6\), temos \(\displaystyle \frac{1}{6} + \frac{4}{3} + c = -6\)\(\displaystyle c = -6 -  \frac{1}{6} - \frac{4}{3} = \frac{-135}{18} = -7.5\) Portanto \(\displaystyle F(x) = \frac{x^6}{6} + \frac{4 x^{3/2}}{3}  - 7.5\)\end{divisionexercise}%
\begin{divisionexercise}{5}{}{}{g:exercise:idp57}%
Encontre a primitiva mais geral da função \(\displaystyle{ \frac{4}{\sqrt[3]{x}} - 6 \sqrt[3]{x^2}}\).%
\par
Primitiva =  \fillin{15.9090909090909}%
\par\smallskip%
\noindent\textbf{\blocktitlefont Solução}.\hypertarget{g:solution:idp58}{}\quad{}Primeiro reescreva a função como segue:%
\begin{equation*}
...
%
\end{divisionexercise}%
\begin{divisionexercise}{6}{}{}{g:exercise:idp59}%
Dada a função%
\begin{equation*}
f''(x) =
5x  -    6
...
com \(f'(  -2) =5\) e \(f(  -2)=4\).%
\par
Encontre \(f'(x) =\) \fillin{13.6363636363636}%
\par
e determine \(f(   3) =\) \fillin{13.6363636363636}%
\par\smallskip%
\noindent\textbf{\blocktitlefont Resposta 1}.\hypertarget{g:answer:idp60}{}\quad{}\(2.5x^{2}+\left(-6\right)x+\left(-17\right)\)%
\par\smallskip%
\noindent\textbf{\blocktitlefont Resposta 2}.\hypertarget{g:answer:idp61}{}\quad{}\(-66.8333333333333\)%
\par\smallskip%
\noindent\textbf{\blocktitlefont Solução}.\hypertarget{g:solution:idp62}{}\quad{}Para resolver esses problemas, usaremos o fato de que a primitiva mais geral de \(f (x)\) é \(F (x) + C\) para alguma constante \(C\). Então, começamos encontrando a primitiva da segunda derivada \(f''(x)\).%
\par
%
\begin{equation*}
f''(x) =    5x  -    6 
...
%
\par
Precisamos descobrir qual é essa constante \(C_1\).  Para isso, vamos usar as informações fornecidas no problema sobre\(f'(-2)\).  A saber, \(f'(  -2) = 5\).  Logo, temos%
\begin{equation*}
\begin{aligned}
...
%
\par
Portanto, \(f'(x) = \frac{5}{2}x^2  -    6x  - 17\).%
\par
Agora, procedemos como antes, desta vez encontrando a primitiva de  \(f'(x)\) e usar informações sobre \(f(3)\) para encontrar \(f(x)\).  A primitive, e o valor de \(C_2\), são obtidos como segue:%
\par
%
\begin{equation*}
...
%
\par
Portanto, a função \(f(x) = \frac{5}{6}x^3 + \frac{-6}{2}x^2  - 17x  - 11.33\) de modo que \(f(3) = -66.83\).%
\end{divisionexercise}%
\begin{divisionexercise}{7}{}{}{g:exercise:idp63}%
Determine a seguinte integral definida \(\displaystyle \int 12\sin\!\left(x\right)-4\cos\!\left(x\right)  \,dx=\) \fillin{9.090909090909092} \(+C\)%
\par\smallskip%
\noindent\textbf{\blocktitlefont Resposta}.\hypertarget{g:answer:idp64}{}\quad{}\(-12\cos\!\left(x\right)-4\sin\!\left(x\right)\)%
\par\smallskip%
\noindent\textbf{\blocktitlefont Solução}.\hypertarget{g:solution:idp65}{}\quad{}\(\displaystyle \int 12\sin\!\left(x\right)-4\cos\!\left(x\right) \,dx= 12 \int \sin(x) \, dx - 4 \int \cos(x) \,dx\)%
\par
\(\quad = -12\cos\!\left(x\right)-4\sin\!\left(x\right) + C\)%
\end{divisionexercise}%
\begin{divisionexercise}{8}{}{}{g:exercise:idp66}%
Considere a função  \(f(t) =  9 \sec ^2(t) - 4 t^ { 2 }\). Seja \(F(t)\) a primitiva de \(f(t)\) com \(F(0) = 0\).%
\par
Então \(F(1.5) =\) \fillin{9.090909090909092}%
\par\smallskip%
\noindent\textbf{\blocktitlefont Resposta}.\hypertarget{g:answer:idp67}{}\quad{}\(122.412779524545\)%
\par\smallskip%
\noindent\textbf{\blocktitlefont Solução}.\hypertarget{g:solution:idp68}{}\quad{}Para resolver este problema, você precisa lembrar que a derivada de \(\rm{tg(t)}\) é \(\sec^2(t)\).  Não é difícil derivar isso usando a identidade \(\rm{tg}(t) = \frac{\rm{sen(t)}}{\cos(t)}\) e a regra de cocientes para derivadas.%
\par
Com isso em mente, encontre a primitiva geral de  \(f(t) =  9\sec^2(t)-{4}t^2\) que será \(F(t) = 9\rm{tg(t)} - \frac{ {4}t^3 }{ 3} + 
C\). Agora usando o fato de \(F(0) = 0\) e que \(\rm{tg}(0) = 0\), devemos ter \(C = 0\).%
\par
...
%
\par
Logo, \(f(x) = -8 \sin x  - 3 x + 0  + 3 \pi\).%
\end{divisionexercise}%
\begin{divisionexercise}{11}{}{}{g:exercise:idp73}%
...
%
\par
\(\displaystyle \int\) \fillin{9.090909090909092} \(dx\) \(=\) \fillin{6.818181818181818} \(+\) \(C\)%
\par\smallskip%
\noindent\textbf{\blocktitlefont Resposta 1}.\hypertarget{g:answer:idp74}{}\quad{}\(\frac{-3x^{2}-18x+8}{\left(3x^{2}-x+5\right)^{2}}\)%
\par\smallskip%
\noindent\textbf{\blocktitlefont Resposta 2}.\hypertarget{g:answer:idp75}{}\quad{}\(\frac{x+3}{3x^{2}-x+5}\)%
\par\smallskip%
\noindent\textbf{\blocktitlefont Solução}.\hypertarget{g:solution:idp76}{}\quad{}Usaremos a Regra do Quociente para computar a derivada.%
\par
%
\begin{equation*}
...
\par
\(\displaystyle \int\left [\frac{\sec\!\left(x\right)+\cos\!\left(x\right)}{3\cos\!\left(x\right)}\right] dx\) \(=\) \fillin{11.3636363636364} \(+\) \(C\)%
\par\smallskip%
...
\end{sbspanel}%
\end{sidebyside}%
\footnotetext[1]{Deslocamentos verticais e horizontais do gráfico de uma função conhecida.\label{g:fn:idp80}}%
...
%
\typeout{************************************************}
\typeout{Subseção 2.2.1 Sugestão de Vídeos}
\typeout{************************************************}
%
...
%
\typeout{************************************************}
\typeout{Subseção 2.4 Sugestão de Vídeos}
\typeout{************************************************}
%
...
%
\typeout{************************************************}
\typeout{Exercícios 2.5 Exercícios}
\typeout{************************************************}
%
